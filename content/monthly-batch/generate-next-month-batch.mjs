...
</style></head><body><main class="poster"><div class="bg"></div><div class="top"><div class="site">app.alzbdh.com</div><img src="${logo}" alt="الزبدة"></div>${isPerfume ? `<div class="product"><img src="${image}" alt=""></div>` : ''}<section class="panel"><div class="eyebrow">${escapeHtml(copy.eyebrow)}</div><h1>${escapeHtml(copy.title)}</h1><div class="subtitle">${escapeHtml(copy.subtitle)}</div><p>${escapeHtml(copy.body)}</p><div class="cta">${escapeHtml(copy.cta)}</div></section></main></body></html>`;
}
