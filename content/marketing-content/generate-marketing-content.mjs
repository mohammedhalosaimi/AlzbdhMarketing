...
.kind{position:absolute;z-index:8;inset-block-start:${isWide ? 112 : 112}px;inset-inline-start:${isWide ? 72 : 58}px;font-size:${isWide ? 20 : 19}px;font-weight:800;color:#DCE9DF;opacity:.88}
...
</style></head><body><main class="poster ${campaign.style}"><div class="top"><div class="site">app.alzbdh.com</div><img src="${logo}" alt="الزبدة"></div><div class="kind">${escapeHtml(campaign.kind)}</div>${bodyFor(campaign, image, title)}</main></body></html>`;
}
...
try {
  for (const campaign of campaigns) {
    for (const [platform, format] of Object.entries(formats)) {
      const dir = path.join(here, campaign.vertical);
...
for (const campaign of campaigns) {
  for (const platform of Object.keys(formats)) {
    rows.push({
      ...
      slug: campaign.slug,
      platform,
      image_path: `content/marketing-content/${campaign.vertical}/images/${formats[platform].dir}/${campaign.slug}.png`,
      text: campaign.captions[platform],
      hashtags: campaign.hashtags[platform],
      suggested_use: campaign.kind
    });
  }
}
...
await fs.mkdir(path.join(here, 'copy'), { recursive: true });
await fs.writeFile(
  path.join(here, 'copy/captions.csv'),
  ['vertical,slug,platform,image_path,text,hashtags,suggested_use', ...rows.map((row) => ['vertical', 'slug', 'platform', 'image_path', 'text', 'hashtags', 'suggested_use'].map((key) => csvEscape(row[key])).join(','))].join('\n') + '\n'
);

await fs.writeFile(
  path.join(here, 'copy/captions-and-hashtags.md'),
...
const contactItems = campaigns.map((campaign) => ({
  file: path.join(here, campaign.vertical, 'images/instagram_1080x1350', `${campaign.slug}.png`),
